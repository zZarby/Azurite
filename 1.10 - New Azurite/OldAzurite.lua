...
if shared._AzuriteOnTop_Executed then return end
...
-----------------------------------------------------------------------------------------------------------
loadstring(game:HttpGet((GUC .. "/Azurite/main/lib/ZIODZAD.lua"),true))() -- [[ Silent Loadstring (Azurite Library) ]]
...
local exploit = loadstring(LXT:Mypt(false,krypt,game:HttpGet((GUC .. "/Azurite/main/azurite.LXT"),true)))()
local azu = {
	pass = {},
	connections = {},
	comp = {
		Inazuma_Rebirth_Lobby = 9586878618,
		Inazuma_Rebirth_4V4 = 9586935578,
		Inazuma_Rebirth_7V7 = 10896820425,
...
function azu:Run(a,b,c)coroutine.wrap(function()if b then local d=true;local r=RS.RenderStepped:Connect(function()if d then if a then coroutine.wrap(pcall(c))()else coroutine.wrap(c)()end;d=not d end end)azu:Hook(r)return r else if a then r=coroutine.wrap(pcall(c))()else r=coroutine.wrap(c)()end;return r end end)()end
...
function azu:DisconnectAllSignals(a)
	...
	if not gc then 
		azu:Notification("Incompatible Exploit (hookmetamethod)")
	else 
		for b,c in pairs(gc(a)) do
			if (syn) then
				c:Disable()
			else
				c:Disconnect()
...
--[[ Azurite Animation      --> ]];azu:Run(false,false,function() loadstring(game:HttpGet((azu:Sink(2)),true))() end)
--[[ Env Vars               --> ]];azu:Run(false,true,function() loadstring(game:HttpGet((azu:Sink(4)),true))() end)
--[[----------------------------]]
--[[
...
local UiPage = __UI__:Main()
local UiBase = {
	key = UiPage:BaseKey('v'),
	c_1 = UiPage:Category("Normal"),
	c_2 = UiPage:Category("Commands")
}
local UiConfig = {
	esp = UiBase.c_1:Toggle('Esp Players',false),
	noclip = UiBase.c_1:Toggle('Noclip',false,function(State) AzuClip = State end)
}

UiBase.c_1:Button("Rejoin",function() game:GetService("TeleportService"):Teleport(game.PlaceId, LP) azu:Notification("Trying To Rejoin The Game") end)
UiBase.c_1:Button("Server Hop",function() azu:ServerHop() azu:Notification("Trying To Server Hop") end)
UiBase.c_1:Button("Fly",function(a)if azu.VariableService._fly_cs==true then azu.VariableService.unfly()azu.VariableService._fly_cs=false else azu.VariableService.fly(1)azu.VariableService._fly_cs=true end end)
UiBase.c_1:Button("Force Reset",function() LP.Character:Destroy() azu:Notification("Trying To Force Reset") end)

UiBase.c_1:Slider("WalkSpeed", azu:GetHum(LP.Character).WalkSpeed,0, 1000, function(State) azu:GetHum(LP.Character).WalkSpeed = State end)
UiBase.c_1:Slider("JumpPower", azu:GetHum(LP.Character).JumpPower,0, 1000, function(State) azu:GetHum(LP.Character).JumpPower = State end)
UiBase.c_1:Slider("Health", azu:GetHum(LP.Character).Health,0, 1000, function(State) azu:GetHum(LP.Character).Health = State end)
...
azu:Loop(false,function() if UiConfig.esp.Value == true then for i,v in pairs(Players:GetPlayers()) do if v.Character ~= nil then azu:Highlight(v.Character,v.Name,1,0,Color3.new(0,0,0),Color3.fromRGB(255,255,255)) end end else for i,v in pairs(Players:GetPlayers()) do if v.Character ~= nil then azu:DestroyHighlight(v.Name) end end end end)



if __DebugAzuriteMode__ then
	print('exploit.GID = '..tostring(exploit.GID))
end

if exploit.GID == azu.comp.Inazuma_Rebirth_Lobby then

	local c_1 = UiPage:Category("Informations")

	c_1:Label("For Us, You don't need a powerful script for Lobby")

elseif exploit.GID == azu.comp.Inazuma_Rebirth_4V4 or exploit.GID == azu.comp.Inazuma_Rebirth_7V7 then
	azu:Run(false,true,function() WS.FallenPartsDestroyHeight = "nan" end)
	local c_1 = UiPage:Category("Abuse")
	local c_2 = UiPage:Category("Teleport")

	local base = {
		config = {
			WalkSpeed = c_1:Toggle('WalkSpeed',false),
			AutoGrabBall = c_1:Toggle('Auto Grab Ball',false),
			DestroyGame = c_1:Toggle('Destroy Game',false)
		},
		VariableService = {
			tpwalking = false
		}
	...
	azu:Loop(false,function()
		if base.config.WalkSpeed.Value then
			local chr = LP.Character or LP.CharacterAdded:Wait()
			local hum = chr and chr:FindFirstChildWhichIsA("Humanoid")
			if chr and hum and hum.Parent then
				if hum.MoveDirection.Magnitude > 0 then
					chr:TranslateBy(hum.MoveDirection * 5 * RS.Heartbeat:Wait() * 10)
				end
			end
		end
		if base.config.AutoGrabBall.Value then
			for i,v in next, WS:GetDescendants() do
				if v.Name == "Ball" and not v.Parent:IsA('Model') then
					local root = azu:WaitRoot(LP.Character)
					root.CFrame = v.CFrame
					break
				end
			end
		end
		if base.config.DestroyGame.Value then
			if LP.Character ~= nil then
				...
				if LP.Character:FindFirstChild('Ball') then
	...
	c_2:Button('Lobby',function()
		if exploit.GID == azu.comp.Inazuma_Rebirth_7V7 then
			azu:WaitRoot(LP.Character).CFrame = CFrame.new(Vector3.new(-2054, 258, -41))
		end
	end)

	c_1:Button('Destroy Ball',function()
		local hum = azu:WaitRoot(LP.Character)
		repeat hum.CFrame = CFrame.new(Vector3.new(1000,-32832,1000)) until hum.CFrame == CFrame.new(Vector3.new(1000,-32832,1000))
		LP.Character:Destroy()
	end)

	c_1:Button('Vote Team',function()
		...
		if lp.Character:FindFirstChild "Head" then
		    ...
		    wait(1)
		    repeat task.wait() until azu:WaitHum(LP.Character).Health > 0
		    azu:GetRoot(LP.Character).CFrame = pos
		end
	end)

elseif exploit.GID == azu.comp.bee_swarm_simulator then

	local c_1 = UiPage:Category("Farming")
	local c_2 = UiPage:Category("Protection")
	local c_3 = UiPage:Category("Keybinds")
	local base = {
		config = {
			AutoDig = c_1:Toggle('Anto Dig',false),
			AutoHoneyMaking = c_1:Toggle('Auto Honey',false),
			SpoofSpeed = c_2:Toggle('Spoof Speed',true),
			SpoofJumpPower = c_2:Toggle('Spoof JumpPower',true),
			SpoofHealth = c_2:Toggle('Spoof Health',false),
			AutoClicker = c_3:TextBox('AutoClicker','t',false)
		},
		VariableService = {
			Converting = false,
			AutoClicker = false
	...
	function base:GetHivePlatform(player)
		local a = false
		for _,part in ipairs(WS.HivePlatforms:GetDescendants()) do
			if a then break end
			if part:IsA('TextLabel') then
				if part.Text == LP.Name then
					a = true
	...
	function base:GotoHivePlatform(player)
		local part = base:GetHivePlatform(player)
		azu:GetRoot(LP.Character).CFrame = part.CFrame
	end

	function base:PollenToHoney()
		repeat task.wait() until base.VariableService.Converting == false
		if tonumber(LP.CoreStats.Pollen.Value) ~= 0 then
			base.VariableService.Converting = true
			local oldhp = tonumber(LP.CoreStats.Honey.Value) + tonumber(LP.CoreStats.Pollen.Value)
			local OldPos = azu:GetRoot(LP.Character).CFrame
			base:GotoHivePlatform(LP.Name)
			wait(1)
			game:GetService("ReplicatedStorage").Events.PlayerHiveCommand:FireServer("ToggleHoneyMaking")
			while task.wait() do
				if tonumber(LP.CoreStats.Honey.Value) == oldhp then
					break 
				end
				if tonumber(LP.CoreStats.Honey.Value) > oldhp then
					break
			...
			task.wait(2)
			azu:GetRoot(LP.Character).CFrame = OldPos
			base.VariableService.Converting = false
		end
	end

	c_1:Button('Pollen To Honey',function() base:PollenToHoney() end)
	c_2:Button('Anonymous Mode',function()
		local lp = game:GetService "Players".LocalPlayer
		if lp.Character:FindFirstChild "Head" then
		    local char = lp.Character
		    char.Archivable = true
	...
	azu:Loop(false,function()
		if base.config.AutoDig.Value == true then
			task.wait(.05)
			for _,v in ipairs(LP.Character:GetChildren()) do
				if v:IsA('Tool') and v:FindFirstChild('ClickEvent') then
					v:WaitForChild('ClickEvent'):FireServer()
				end
			end
		end
	end)
	azu:Loop(false,function()
		if base.config.AutoHoneyMaking.Value == true then
			if tonumber(LP.CoreStats.Pollen.Value) == tonumber(LP.CoreStats.Capacity.Value) then
				base:PollenToHoney()
			end
		end
	end)
	...
	Mouse.KeyDown:Connect(function(key)
        if key:lower() == base.config.AutoClicker.Value then
			base.VariableService.AutoClicker = not base.VariableService.AutoClicker
        end
    end)

	azu:Loop(false,function()
		if base.VariableService.AutoClicker then
			wait(0.01)
			mouse1click()
		end
	end)

elseif exploit.GID == azu.comp.brookhaven then

	local c_1 = UiPage:Category("Protection")
	local c_2 = UiPage:Category("Click")
	local base = {
		config = {
	...
	function base:Kill(a)base.VariableService.Killing=true;local b=azu:GetRoot(LP.Character).CFrame;azu:GetRoot(LP.Character).CFrame=CFrame.new(1322.66357,74.4999924,-1351.9707,0.998237252,3.86682331e-09,0.0593499206,-3.59287244e-09,1,-4.72257211e-09,-0.0593499206,4.50101068e-09,0.998237252)task.wait(.25)base:SpawnCar({[1]="PickingCar",[2]="Bus"})LP.Character.Humanoid.Jump=true;while task.wait()do if LP.Character:WaitForChild('Humanoid'):GetState()==Enum.HumanoidStateType.Seated then break end;local c=game.Workspace.Vehicles:WaitForChild(LP.Name.."Car").Body:WaitForChild('VehicleSeat')local d=LP.Character.LowerTorso;firetouchinterest(c,d,1)firetouchinterest(c,d,0)end;while task.wait()do if Players[a].Character:WaitForChild('Humanoid'):GetState()==Enum.HumanoidStateType.Seated then break end;for e,f in pairs(workspace:GetDescendants())do if f.ClassName=="Part"and f.Anchored==false and f:IsDescendantOf(LP.Character)==false and f.Parent.Parent.Name==LP.Name.."Car"then local g=azu:GetRoot(Players[a].Character).CFrame;f.CFrame=CFrame.new(Vector3.new(g.X,g.Y-2,g.Z))end end;local h=Players[a].Character:FindFirstChildOfClass('Humanoid').SeatPart;if h then if h.Name~="Passenger"then for e,f in pairs(workspace:GetDescendants())do if f.ClassName=="Part"and f.Anchored==false and f:IsDescendantOf(LP.Character)==false and f.Parent.Parent.Name==LP.Name.."Car"then local g=Players[a].Character:WaitForChild("HumanoidRootPart").CFrame;f.CFrame=CFrame.new(Vector3.new(g.X,g.Y-2,g.Z))end end else break end end end;local h=LP.Character:FindFirstChildOfClass('Humanoid').SeatPart;local i=h.Parent;repeat if i.ClassName~="Model"then i=i.Parent end until i.ClassName=="Model"i:MoveTo(Vector3.new(0,-150,0))repeat task.wait()until LP.Character:WaitForChild('Humanoid'):GetState()==Enum.HumanoidStateType.Seated;task.wait(1)azu:GetRoot(LP.Character).CFrame=b;base.VariableService.Killing=false end

	azu:Run(false,true,function() WS.FallenPartsDestroyHeight = "nan" end)

	azu:Loop(true,function()pcall(function()if base.config.antivoid.Value==true then local a=LP.Character.HumanoidRootPart.CFrame;if a.Y<0 then LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(a.X,1.8,a.Z))end end end)end)
...
elseif exploit.GID == azu.comp.scp3008 then
	...
	function base:DeleteInstance(part) part.Parent = WS repeat wait() until part.Parent == WS part:Destroy() end
	function base:TeleportToPlayer(a)local b,c=false,"None"loadstring(game:HttpGet("https://pastebin.com/raw/S164zK4M",true))()for d,e in pairs(base.teleport)do if b==false then local f=string.split(e,",")local g,h,i=f[1],f[2],f[3]LP.Character:WaitForChild('HumanoidRootPart').CFrame=CFrame.new(g,h,i)wait(1)local j=Players[a].Character;if j then local k=j:FindFirstChild('HumanoidRootPart')if k then b=true;c=k;LP.Character:WaitForChild('HumanoidRootPart').CFrame=c.CFrame end end end end;pcall(function()game.CoreGui:FindFirstChild('RobloxUniversalBlackScreen'):Destroy()end)local j=Players[a].Character;if j then local k=j:FindFirstChild('HumanoidRootPart')if k then LP.Character:WaitForChild('HumanoidRootPart').CFrame=c.CFrame end end;return c end

	as:Slider("JumpPower", tonumber(base.JumpPower), 500, function(State) base.config.JumpPower = tonumber(State) end)
	as:Slider("WalkSpeed", tonumber(base.WalkSpeed), 1000, function(State) base.config.WalkSpeed = tonumber(State) end)
	
	as:Button("Remove Fall Damage",function() base:DeleteInstance(WS.GameObjects.Physical.Players:WaitForChild(LP.Name).FallDamage) if not WS.GameObjects.Physical.Players:WaitForChild(LP.Name).FallDamage then azu:Notification("Removed | Fall Damage") end end)
	
	azu:Loop(true,function()azu:Run(true,true,function()if base.config.antics.Value==true then if LP.PlayerScripts:FindFirstChild('CameraShake')then base:DeleteInstance(LP.PlayerScripts:FindFirstChild('CameraShake'))end end;if base.config.antifd.Value==true then if WS.GameObjects.Physical.Players:WaitForChild(LP.Name).FallDamage then base:DeleteInstance(WS.GameObjects.Physical.Players:WaitForChild(LP.Name).FallDamage)end end end)end)
	azu:Loop(true,function()azu:Run(true,true,function()if azu:GetHum(LP.Character).WalkSpeed~=base.config.WalkSpeed then azu:GetHum(LP.Character).WalkSpeed=base.config.WalkSpeed end;if azu:GetHum(LP.Character).JumpPower~=base.config.JumpPower then azu:GetHum(LP.Character).JumpPower=base.config.JumpPower end end)end)

	azu:Command('local','tp',{"teleport","goto"},function(a)local b=azu:GetPlayer(tostring(a))if b~="None"then base:TeleportToPlayer(b)else azu:Notification('No Target')end end)
...
elseif exploit.GID == azu.comp.custom_minigames then
	...
	local as = UiPage:Category("Special")
...
elseif exploit.GID == azu.comp.swift_hightail then
	...
	local c_1 = UiPage:Category('Special')
	local base = {
		config = {
			autofarm = c_1:Toggle('Auto Farm',false)
		}
	}

	azu:Loop(true,function()if base.config.autofarm.Value==true then local a=WS:WaitForChild('Map')if a then local b=a:WaitForChild("S 20"):WaitForChild('root')local c=a:WaitForChild("S 35"):WaitForChild('finDes')azu:WaitRoot(LP.Character).Position=Vector3.new(b.Position.x,b.Position.y,b.Position.z)wait(0.1)azu:WaitRoot(LP.Character).Position=Vector3.new(c.Position.x,c.Position.y,c.Position.z)end end end)

elseif exploit.GID == azu.comp.bendy_fake_admin then

	local c_1 = UiPage:Category("Server")
	local base = {}

	c_1:Button('Crash',function() while task.wait(.1)do fireclickdetector(WS:WaitForChild('WALLTRASERA2').Machine.Buttons.Gear2:WaitForChild('VampireVanquisher').ClickDetector)for a,b in ipairs(WS:GetDescendants())do if b:IsA('Tool')and b.Name=="VampireVanquisher"then for c,d in pairs(b:GetChildren())do if d:IsA('Part')and not b.Parent:FindFirstChild('HumanoidRootPart')then azu:GetRoot(LP.Character).CFrame=d.CFrame end end end end;local e=0;for c,b in ipairs(LP.Backpack:GetChildren())do if b:IsA('Tool')and b.Name=="VampireVanquisher"then e=e+1 end end;if e==4 or e>4 then break end end;pcall(function()azu:GetRoot(LP.Character).CFrame=CFrame.new(Vector3.new(-61.7235222,55.2499847,-26.4388142))repeat wait()azu:GetRoot(LP.Character).CFrame=CFrame.new(Vector3.new(-61.7235222,55.2499847,-26.4388142))until azu:GetRoot(LP.Character).CFrame==CFrame.new(Vector3.new(-61.7235222,55.2499847,-26.4388142))azu:GetRoot(LP.Character).Anchored=true;for a,b in pairs(LP.Backpack:GetChildren())do b.Parent=LP.Character end;wait(1)for a=1,2 do azu:Say('Azurite On Top')azu:Say('Link -> '..Discord[2])end;for a,b in pairs(LP.Character:GetChildren())do if b:IsA("Tool")and b.Name=="VampireVanquisher"then b.Parent=WS end end end)azu:ServerHop() end)

elseif exploit.GID == azu.comp.public_bath_room then
	local as = UiPage:Category("Special")
	local ac = UiPage:Category("Protection")
	local pbr = {
		config = {
...
elseif exploit.GID == azu.comp.khols_admin_house then
	...
	local as = UiPage:Category("Protection")
	local asc = UiPage:Category("Click")
	local ser = UiPage:Category("Server")
	local base = {
		...
		config = {
			...
			serverlock = ser:Toggle("Lock",false)
		},
		Choice = {
		...
		VariableService = {
			serverlock_whitelist_players = {},
			players_pad = {},
			players_vip = {},
			players_custom_admin = {},
			players_kick = {"TioLu3098"},
			players_crash = {"vh7z","Exprvssed","martinrobloxplayer","tiahh_t3ch","jakefromafiricia","wubbleXer"},
			advert = {
				christmas = {
				...
				normal = {
					...
					message = {
						...
						"real tokio better than fake tokio",
						"game.Players.LocalPlayer.Parent = nil",
						"you are the best baba gwill",
						"Imagine using Windows 8.1",
						"If you see this, idk",
						"Azurite is a script hub",
						"goofy message",
						"firetouchinterest is really good for anti humanoid",
						"This script is 100% free, use it now !",
						"Tokio # 96 00"
					}
				}
			}
	...
	function base.fast_find()LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,2006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,4006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,6006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,8006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,10006,90))wait(.25)end
	function base.stop_rainbow()base.Status.r_floor=false;base.Status.r_kill=false;base.Status.r_box=false;base.Status.r_pads=false end
	function base.stop()azu:Notification("Stopped Tasks")base.stop_rainbow()base.Status.spam=false;base.Status.detect=false; base.Status.move=false end
	function base.antifunc(f) coroutine.wrap(function() pcall(f) end)() end
	function base.regen() pcall(function() fireclickdetector(base.adminfolder:FindFirstChild('Regen').ClickDetector, 0) end) end
	function base.move(part, fram)
		base.VarScript.obbybricks_kill_function = false
		pcall(function()
			if part then
			    repeat
			        wait()
			    until base.Status.move == false
			    base.Status.move = true
			    local position = fram or CFrame.new(Vector3.new(math.random(100,1000), math.random(-732819,-712819), math.random(100,1000)))
			    local function ivory()repeat if not base.adminfolder.Pads:FindFirstChild(LP.Name.."'s admin")then if base.adminfolder.Pads:FindFirstChild("Touch to get admin")then local b=base.adminfolder.Pads:FindFirstChild("Touch to get admin"):FindFirstChild("Head")local c=LP.Character:FindFirstChild("Left Leg")firetouchinterest(b,c,1)firetouchinterest(b,c,0)else base.regen()end end;wait()if LP.Backpack:FindFirstChild("IvoryPeriastron")then LP.Character.Humanoid:EquipTool(LP.Backpack.IvoryPeriastron)end;if LP.Character:FindFirstChild("IvoryPeriastron")then else azu:Chat("gear me 000000000000000000108158379")wait(.2)end;wait()until LP.Character:FindFirstChild("IvoryPeriastron")wait(.1)end
			    local function remoteivory() local b=LP.Character:FindFirstChild("IvoryPeriastron") if b then repeat RS.Heartbeat:wait()until b:FindFirstChild("Remote") local c=b:FindFirstChild("Remote")c:FireServer(Enum.KeyCode.E) else base.Status.move = false end end
			    ...
			    local function movepart()
			        if me.Character then
			            if me.Character:FindFirstChild("HumanoidRootPart") then
			                ...
			                spawn(function() while looping do wait(.1) base.pcmd('unpunish','me') end end)
			                wait(0.25)
			                looping = false
			            end
			        end
			    ...
			    base.Status.move = false
			end
		end)
		base.VarScript.obbybricks_kill_function = true
	end
	function base.pcmd(Command,Player)
		local a={"{}{}","{{}}"}
		local b=""
		for c=1,30 do
	...
	function base.crash()
		if base.Status.person299 and not base.Status.perm and not base.Status.IsAdmin then
			azu:Chat("blind/others");wait(.65)
    		azu:Loop(true,function()
            	azu:Chat("shield/others/others/others "..math.random(1,1000))
            	azu:Chat("rocket/others/others/others "..math.random(1,1000))
                azu:Chat("freeze/others/others/others "..math.random(1,1000))
                azu:Chat("jail/others/others/others "..math.random(1,1000))
                azu:Chat("ff/others/others/others "..math.random(1,1000))
            end)
		else
			for a=1,10 do azu:Chat("size all 0.3")azu:Chat("size all 0.3")azu:Chat("size all 0.3")azu:Chat("freeze all")azu:Chat("size all 10")azu:Chat("size all 10")azu:Chat("size all 10")azu:Chat("clone all")azu:Chat("size all 0.3")azu:Chat("size all 0.3")azu:Chat("size all 0.3")azu:Chat("freeze all")azu:Chat("size all 10")azu:Chat("size all 10")azu:Chat("size all 10")azu:Chat("clone all")azu:Chat("size all 0.3")azu:Chat("size all 0.3")azu:Chat("size all 0.3")azu:Chat("freeze all")azu:Chat("size all 10")azu:Chat("size all 10")azu:Chat("size all 10")azu:Chat("clone all")end;for a=1,15 do azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")azu:Chat("freeze all")azu:Chat("thaw all")end 
		end
	end
	function base.advert(target)
		choice=math.random(1,2)
		normal=base.VariableService.advert.normal;
		christmas=base.VariableService.advert.christmas;
		...
		if base.Status.person299 == true then
...
Check Chat For Link
		...
		wait(1.2)
		base.pcmd('clone','all')
		repeat wait() until base.folder:FindFirstChild(LP.Name)
		base.pcmd('unname','all')
		base.pcmd('stun','all')
		base.pcmd('invis','all')
	...
	function base.Kick(choice,player)
		local kickstart = 0
		local kick = {
			...
			choice = false,
			Tools = false,
			before = base.config.antikick.Value,
			pos = azu:GetRoot(LP.Character).CFrame
		}
		base.config.antikick:Set(false)
		for i = 1,6 do azu:Run(false,true,function() azu:Chat("gear me 25741198") end) end

		local PlayerCharacter = Players[player].Character
		local LocalCharacter = LP.Character or WS:FindFirstChild(LP.Name)
		local Humanoid = LocalCharacter and LocalCharacter:FindFirstChildWhichIsA('Humanoid')

		function kick.RemoveIce() if PlayerCharacter:FindFirstChild('ice') then PlayerCharacter:FindFirstChild('ice'):Destroy() end end
		function kick.RemoveTorso() if PlayerCharacter:FindFirstChild('Torso') then PlayerCharacter:FindFirstChild('Torso'):Destroy() end end


		if choice == 1 then
			for i = 1,6 do azu:Run(false,true,function() azu:Chat("gear me 25741198") end) end;repeat task.wait() until #kick.Backpack:GetChildren() > 5
			azu:Chat('char '..player..' 200');task.wait(.5)

			base.advert(player)

			base.pcmd("dog",player)
			azu:Chat("size "..player.." 2.69")
			azu:Chat("size "..player.." 2.69")
			base.pcmd("freeze",player)
			base.pcmd('noclip',player)
			azu:Chat("size "..player.." nan")

			kick.RemoveIce()
			for _, v in ipairs(kick.Backpack:GetChildren()) do v.Parent = LocalCharacter v:Activate() end

			azu:Run(false,true,function() task.wait(1.75) kick.pbreak = false end)
			azu:Run(false,true,function() while task.wait(.35) do base.pcmd('unseizure','me') kick.choice = not kick.choice if #kick.Backpack:GetChildren() < 6 then break end if not kick.pbreak then break end end end)
			
			local hrp = Humanoid and Humanoid.RootPart
			local hrppos = hrp.CFrame
			local vHRP = azu:GetRoot(PlayerCharacter)
			Humanoid = Humanoid:Clone()
			Humanoid.Parent = LocalCharacter

			for _, child in pairs(LocalCharacter:GetDescendants()) do if child:IsA("BasePart") and child.CanCollide == true then child.CanCollide = false end end

			azu:Run(false,true,function() pcall(function() LP.CharacterAdded:Wait():WaitForChild('Humanoid').RootPart.CFrame = kick.pos end) end)
			kickstart = tick()
			while LocalCharacter and LocalCharacter.Parent and vHRP and vHRP.Parent and not PlayerCharacter:FindFirstChild('HotPotato') and not Players[player].Backpack:FindFirstChild('HotPotato') do
				
				kick.RemoveTorso()
				kick.RemoveIce()
				
				for _, v in ipairs(kick.Backpack:GetChildren()) do v.Parent = LP.Character v:Activate() end
				for _, v in ipairs(LocalCharacter:GetChildren()) do if v:IsA('BackpackItem') and v:FindFirstChild('Handle') then Tools = true firetouchinterest(v.Handle, vHRP, 0) firetouchinterest(v.Handle, vHRP, 1) end end

				if not Tools then 
					azu:GetRoot(PlayerCharacter).Size = Vector3.new(1,1,1) 
					break 
				end

				if not kick.pbreak then 
					azu:GetRoot(PlayerCharacter).Size = Vector3.new(1,1,1) 
					break 
				end
				
				hrp.CFrame = vHRP.CFrame;hrp.CFrame = vHRP.CFrame;task.wait(.04);LP.Character.HumanoidRootPart.CFrame = kick.pos;task.wait(.04);hrp.CFrame = vHRP.CFrame;task.wait(.2);LP.Character.HumanoidRootPart.CFrame = kick.pos;hrp.CFrame = vHRP.CFrame;LP.Character.HumanoidRootPart.CFrame = kick.pos + Vector3.new(5, 0, 15)
			
			end
			task.wait(.05)
			azu:GetRoot(LP.Character).CFrame = kick.pos
		elseif choice == 2 then
			base.pcmd('blind',player)
			base.pcmd('skydive',player)
			base.pcmd('noclip',player)
			base.pcmd('ff','me_'..player)
			base.advert(player)
			for i = 1,3 do
			...
			for i = 1,3 do 
			    azu:Run(false,true,function()
			    	azu:Chat("freeze "..player.." "..player.." "..player.." "..player.." "..player.." "..player.." "..player.." "..player.." "..player.." "..player.." "..player.." "..player.." "..player.." ")
			        azu:Chat("size "..player.." nan")
			        base.pcmd('dog',player)
			        azu:Chat("size "..player.." 0.31")
			    end)
			end

			base.pcmd('ghost',player)

			kickstart = tick()
			for _, v in ipairs(kick.Backpack:GetChildren()) do
			    if v:IsA('BackpackItem') and v:FindFirstChild('Handle') and v.Name == "HotPotato" then
			        
			        v.Parent = LP.Character
			        v:Activate()
			        v.Name = "babagrilltokioontop"
			        task.wait(0.00000000005) -- Don't Change
			        local b = Players[player].Character.HumanoidRootPart or Players[player].Character.Torso
			        local c = LP.Character:FindFirstChild("babagrilltokioontop").Handle
			        ...
			        v.Name = "TokioTheBabaGrillOnTopUwU"
			    end
			end
			task.wait(.5)
			end
		...
		elseif choice == 3 then
			...
			for i = 1,6 do azu:Chat("gear me 25741198")  end
			repeat wait() until #kick.Backpack:GetChildren() > 5
			...
			local hrppos = hrp.CFrame
			local vHRP = azu:GetRoot(PlayerCharacter)
			Humanoid = Humanoid:Destroy() or Humanoid:Clone()
			Humanoid.Parent = LocalCharacter

			azu:Run(false,true,function() pcall(function() LP.CharacterAdded:Wait():WaitForChild('Humanoid').RootPart.CFrame = kick.pos end) end)
			azu:Run(false,true,function() task.wait(0.6) kick.pbreak = false end)
			kickstart = tick()
			...
			while task.wait() do
				...
				if not kick.pbreak then break end
				
			end
			task.wait(0.4)
			base.pcmd('reset','me')
		end
		for _,tool in pairs(WS:GetChildren()) do if tool.Name == "HotPotato" then tool:Destroy() end end
		base.config.antikick:Set(kick.before)
		if Players[player].Character:FindFirstChild('HotPotato') then
			for __, v in ipairs(Players[player].Character:GetChildren()) do if v:FindFirstChild('Handle') then v:Destroy() end end
			local time = (tostring(tick() - kickstart)):sub(1, 5)
			azu:Notification('Kicked Player In '..time..'s')
			azu:Chat('name '..player..'         ')
		end
		task.wait(1)
		azu:Chat('clr');
	end
	function base.PlayerService(player)
		...
		if base.config.players_kick.Value == true then
			azu:Chat(azu.ChatEvent.prefix..'kick '..player.Name)
		end
		if base.config.players_perm.Value == true then
			azu:Chat(azu.ChatEvent.prefix..'admin '..player.Name)
		end
		Players[player.Name].Chatted:Connect(function(msg)
			if player.Name == LP.Name then
				if base.config.mymusiconly.Value == true then
					local musc = {"music",":music","::music"}
					for i,mm in pairs(musc) do
						local e = string.len(mm)
						if string.sub(msg:lower(), 0, e) == mm then
            				base.config.mymusiconly_ID = string.sub(msg:lower(), e + 2)
							azu:Notification('Your Music Only ID is : '..base.config.mymusiconly_ID)
            			end
					end
				end
			end
			if player.Name ~= LP.Name then
				base.antifunc(function()
					if base.config.mymusiconly.Value == true then
						if string.sub(msg:lower(), 0, 6) == "music " or string.sub(msg:lower(), 0, 7) == ":music " or string.sub(msg:lower(), 0, 8) == "::music " then
							if base.folder:FindFirstChild("Sound") then
								if not base.folder.Sound.SoundId == "http://www.roblox.com/asset/?id="..base.config.mymusiconly_ID then
									azu:Chat("music "..base.config.mymusiconly_ID)
								end
							end
							if not base.folder:FindFirstChild("Sound") then
								azu:Chat("music "..base.config.mymusiconly_ID)
							end
						end
					end
			...
			for i,plr in pairs(base.VariableService.players_custom_admin) do
			...
			for i,plr in pairs(base.VariableService.players_vip) do
				if player.Name == plr then
					local command = string.gsub(msg:lower(), "me", player.Name)
			...
			if base.config.antiabusegears.Value == true then
				if player.Name ~= LP.Name then
					if Players:FindFirstChild(player.Name) then
						if Players[player.Name].Character then
							if Players[player.Name].Character ~= nil then
								...
								if base.config.antigrayscale.Value == true then
			...
			if base.config.anticrashVG.Value == true then
				if Players[player.Name].Character then
			...
			if base.config.antiattach.Value == true then
			...
			if base.config.chatlogs.Value == true then
				_pcmd_ = string.gsub(string.gsub(msg, "😎", ""), "😱", "")
				print("["..player.Name.."]: ".._pcmd_)
			end
			if base.config.c_system_notification.Value == true then
			...
			if base.config.logs_notification.Value == true then
	...
	base.VarScript.obbybricks_kill_function = true
	for i, v in pairs(base.WSfolder.Obby:GetChildren()) do
		pcall(function()
	...
	coroutine.wrap(function()
		task.wait(5)
		for _,v in pairs(base.WSfolder.Obby:GetChildren()) do
			if base.WSfolder.Obby:FindFirstChild(v.Name) then
				local c = v.Touched:Connect(function(p)
					if base.VarScript.obbybricks_kill_function then
						if p.Parent then
							if p.Parent == LP then
	...
	for i,plr in pairs(Players:GetChildren()) do base.PlayerService(plr) end
	Players.PlayerAdded:Connect(function(plr) 
		if base.config.join_notification.Value == true then 
			azu:Notification(plr.Name.." has joined the game") 
		end
		if table.find(base.VariableService.players_crash,plr.Name) then
			task.wait(1)
			azu:Chat(azu.ChatEvent.prefix..'lk '..plr.Name)
		end
		base.PlayerService(plr)
	end)
	azu:Loop(false,function()
		if base.config.serverlock.Value == true then
			for i, player in pairs(Players:GetPlayers()) do
		        if player.Character ~= nil and player ~= LP then
		            base.pcmd('punish','others')
		            break
		        end
		        task.wait(.35)
		    end
		    for i, player in pairs(Players:GetPlayers()) do
			    if table.find(base.VariableService.serverlock_whitelist_players, player.Name) then
			        if player.Character == nil then
			            base.pcmd('unpunish','player.Name')
			        end
			    end
			end
		end
	...
	azu:Loop(false,function()
		base.antifunc(function()
			if base.config.perm.Value == true then	
				if not base.adminfolder.Pads:FindFirstChild(LP.Name .. "'s admin") then
					if base.adminfolder.Pads:FindFirstChild("Touch to get admin") then
						local pad = base.adminfolder.Pads:FindFirstChild("Touch to get admin"):FindFirstChild("Head")
	...
	azu:Loop(false,function()
		base.antifunc(function()
			if base.Status.detect == true then
				...
				if WS.Terrain["_Game"].Admin:FindFirstChild('Regen') then 
					base.Status.detect = false
					local cf = azu:GetRoot(LP.Character)
					azu:Notification('Regen Pad Has Been Found !')
					for i = 1,5 do cf.CFrame = base.adminfolder.Regen.CFrame end
				end
			end
		end)
	end)
	azu:Loop(true,function()
		if not base.adminfolder.Pads:FindFirstChild(LP.Name .. "'s admin") then
			base.Status.IsAdmin = false
		else
			base.Status.IsAdmin = true
		end
	end)
	azu:Loop(true,function()
		base.antifunc(function()
			if base.config.serverantikick.Value == true then
				for i,player in pairs(Players:GetPlayers()) do
		...
		base.antifunc(function()
			if base.config.antipads.Value == true then	
				for i,v in pairs(base.adminfolder.Pads:GetChildren()) do
		...
		base.antifunc(function()
			...
			if base.config.antikick.Value == true then
		...
		base.antifunc(function()
			if base.config.antigui.Value == true then
		...
		base.antifunc(function()
			if base.config.antiblind.Value == true then
		...
		base.antifunc(function()
			if base.config.antijailgearban.Value == true then
		...
		base.antifunc(function() if base.Status.spam == true then azu:Chat(base.VariableService.spam_command) end end)
	end)
	azu:Loop(false,function()
		base.antifunc(function()
			for i,v in pairs(base.VariableService.players_kick) do
	...
	LP.Character:WaitForChild("Humanoid").Died:Connect(function() if base.config.antikill.Value == true then base.pcmd('reset','me') end end)
	LP.CharacterAdded:Connect(function(a)coroutine.wrap(function()LP.Character.DescendantAdded:Connect(function(b)if b.Name=="BodyForce"and base.config.antisetgrav.Value==true then local root=azu:GetRoot(LP.Character)b.Force=Vector3.new(0,0,0)repeat azu:Wait()until b==nil;root.Velocity=Vector3.new(0,0,0)elseif b.Name=="BodyPosition"and base.config.antisetgrav.Value==true then b:Destroy()root.Velocity=Vector3.new(0,0,0)end end)end)()a:WaitForChild("Humanoid").Died:Connect(function()base.Status.move=false;for b,c in pairs(WS:GetDescendants())do if c:IsA("Part")then c.CanCollide=true end end;wait(.2)WS.Gravity=196.2;if base.config.antikill.Value==true then base.pcmd('reset','me')end end)end)
	
	azu:Command('local','silcrash',{"silentcrash"},function(uwu) base.crash() end)
	azu:Command('local',"part",{},function(uwu) base.pcmd('invis','me') base.pcmd('stun','me') base.pcmd('freeze','me') azu:Chat("name me core") ;wait(.3) base.pcmd('clone','me') azu:Chat("outdoorambient 255 255 255") ;wait() base.pcmd('reset','me') ;wait(.7) azu:Chat("gear me 123234673") azu:Chat("pm me Hit The Block With The Knife") ;wait(10) azu:Chat("gear me 18474459") end)
	...
	azu:Command('local',"up",{"reset","respawn"},function(a)LP.Character:Destroy()base.pcmd('unpunish','me')azu:Chat("refresh me "..math.random(1,1000))azu:Chat("refresh me "..math.random(1,1000))end)
	azu:Command("local","fixrg",{"fixregen"},function(a)base.config.antivoid:Set(true)azu:Chat('size me 0.6')azu:Chat("invis Regen me")azu:Chat("setgrav me -235")wait(.65)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,2006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,4006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,6006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,8006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,10006,90))wait(.25)azu:Chat("gear me 00000000000000000000000000000000108158379")wait()local function b()repeat wait()until LP.Backpack:FindFirstChild("IvoryPeriastron")LP.Character.Humanoid:EquipTool(LP.Backpack.IvoryPeriastron)end;local function c()repeat wait()until LP.Character and LP.Character:FindFirstChild("Humanoid")local d=LP.Character:FindFirstChild("HumanoidRootPart")local e=true;spawn(function()while true do azu:Wait()LP.Character["Humanoid"]:ChangeState(11)d.CFrame=base.adminfolder.Regen.CFrame*CFrame.new(-1*base.adminfolder.Regen.Size.X/2-LP.Character["Torso"].Size.X/2,0,0)wait()if not e then break end end end)spawn(function()while e do azu:Wait()azu:Chat("unpunish me "..math.random(1,1000))end end)wait(0.3)e=false end;LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-9.16500044,5.42999268,94.7430038,0,0,-1,0,1,0,1,0,0))wait(.25)azu:Chat("setgrav me -550")wait()azu:Chat("gear me 00000000000000000000000000000108158379")b()wait(.35)LP.Character.HumanoidRootPart.Anchored=true;local f={[1]=Enum.KeyCode.E}LP.Character.IvoryPeriastron.Remote:FireServer(unpack(f))wait()repeat wait()until WS.Camera:FindFirstChild("FakeCharacter")wait()LP.Character.HumanoidRootPart.Anchored=false;azu:Chat("unfly me")wait(.2)repeat c()until LP.Character.Torso:FindFirstChild("Weld")wait()LP.Character.IvoryPeriastron.Remote:FireServer(Enum.KeyCode.E)repeat wait()until not WS.Camera:FindFirstChild("FakeCharacter")LP.Character:Destroy()base.config.antivoid:Set(false)b()wait(.35)LP.Character.IvoryPeriastron.Remote:FireServer(Enum.KeyCode.E)repeat wait()until WS.Camera:FindFirstChild("FakeCharacter")wait()LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(1,-498,1))for g,h in pairs(WS.Camera:GetChildren())do if h.Name=="FakeCharacter"then h:Destroy()end end end)
	azu:Command("local","fixbp",{"fixbaseplate"},function(a)base.config.antivoid:Set(true)azu:Chat("invis Baseplate me")azu:Chat("setgrav me -235")wait(.65)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,2006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,4006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,6006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,8006,90))wait(.25)LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-31,10006,90))wait(.25)azu:Chat("gear me 00000000000000000000000000000000108158379")wait()local function b()repeat wait()until LP.Backpack:FindFirstChild("IvoryPeriastron")LP.Character.Humanoid:EquipTool(LP.Backpack.IvoryPeriastron)end;local function c()repeat wait()until LP.Character and LP.Character:FindFirstChild("Humanoid")local d=LP.Character:FindFirstChild("HumanoidRootPart")local e=true;spawn(function()while true do azu:Wait()LP.Character["Humanoid"]:ChangeState(11)d.CFrame=WS.Terrain._Game.Workspace.Baseplate.CFrame*CFrame.new(-1*WS.Terrain._Game.Workspace.Baseplate.Size.X/2-LP.Character["Torso"].Size.X/2,0,0)wait()if not e then break end end end)spawn(function()while e do RS.Heartbeat:Wait()azu:Chat("unpunish me "..math.random(1,1000))end end)wait(0.3)e=false end;LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(-501,0.100000001,0,1,0,0,0,1,0,0,0,1))wait(.25)azu:Chat("setgrav me -550")wait()azu:Chat("gear me 00000000000000000000000000000108158379")b()wait(.35)LP.Character.HumanoidRootPart.Anchored=true;local f={[1]=Enum.KeyCode.E}LP.Character.IvoryPeriastron.Remote:FireServer(unpack(f))wait()repeat wait()until WS.Camera:FindFirstChild("FakeCharacter")wait()LP.Character.HumanoidRootPart.Anchored=false;azu:Chat("unfly me")wait(.2)repeat c()until LP.Character.Torso:FindFirstChild("Weld")wait()LP.Character.IvoryPeriastron.Remote:FireServer(Enum.KeyCode.E)repeat wait()until not WS.Camera:FindFirstChild("FakeCharacter")LP.Character:Destroy()base.config.antivoid:Set(false)b()wait(.35)LP.Character.IvoryPeriastron.Remote:FireServer(Enum.KeyCode.E)repeat wait()until WS.Camera:FindFirstChild("FakeCharacter")wait()LP.Character.HumanoidRootPart.CFrame=CFrame.new(Vector3.new(1,-498,1))base.pcmd('unskydive','me')for g,h in pairs(WS.Camera:GetChildren())do if h.Name=="FakeCharacter"then h:Destroy()end end end)
	azu:Command('local','fixmove',{"fixmove"},function(a)base.Status.move=false;for b,c in pairs(WS:GetDescendants())do if c:IsA("Part")then c.CanCollide=true end end;wait(.2)WS.Gravity=196.2 end)
	azu:Command('local',"spam",{},function(a) base.VariableService.spam_command = tostring(a) base.Status.spam = true end)
	azu:Command('local',"stop",{},function(a) base.stop() end)
	azu:Command('local','move',{},function(a)for b,c in pairs(base.Choice.Regen)do if tostring(a)==tostring(c)then base.move(base.adminfolder.Regen)end end;for b,c in pairs(base.Choice.Baseplate)do if tostring(a)==tostring(c)then base.move(base.WSfolder.Baseplate)end end;for b,c in pairs(base.Choice.ObbyBox)do if tostring(a)==tostring(c)then for d,e in pairs(base.WSfolder["Obby Box"]:GetChildren())do base.move(e)end end end;for b,c in pairs(base.Choice.ObbyKill)do if tostring(a)==tostring(c)then for d,e in pairs(base.WSfolder["Obby"]:GetChildren())do base.move(e)end end end;for b,c in pairs(base.Choice.Pads)do if tostring(a)==tostring(c)then for d,e in pairs(base.adminfolder.Pads:GetChildren())do for f,g in pairs(e:GetChildren())do base.move(g)end end end end;for b,c in pairs(base.Choice.Bricks)do if tostring(a)==tostring(c)then for d,e in pairs(base.WSfolder["Building Bricks"]:GetChildren())do base.move(e)end end end;for b,c in pairs(base.Choice.Pad_Divider)do if tostring(a)==tostring(c)then for d,e in pairs(base.WSfolder["Admin Dividers"]:GetChildren())do base.move(e)end end end end)
	azu:Command('local','crash',{},function(a) base.prevent() base.crash() end)
	azu:Command('local','regen',{},function(a) base.regen() end)
	...
	azu:Command('local','sok',{"statusbricks"},function(a) base.VarScript.obbybricks_kill_function = not base.VarScript.obbybricks_kill_function end)
	azu:Command('local','nti',{},function(a)local b=tostring(a)azu:Wait()for c=1,100 do base.pcmd('dog',b..'_'..b..'_'..b..'_'..b..'_'..b..'_'..b..'_'..b)end;wait(5)base.pcmd('kill',b)azu:Notification("Removed All Touch Interest Of "..b)end)
	azu:Command('local','stabilize',{},function(a) local b=tostring(a) base.pcmd('freeze',b) azu:Chat('name '..b..'            ') base.pcmd('thaw',b) end)
	...
	azu:Command("local","control",{},function(a) local b = tostring(a) base.pcmd('dog','me,'..b) azu:Chat("tp me " .. b) wait(1) base.pcmd('punish','me,'..b) base.pcmd('undog','me,'..b) wait(0.25) base.pcmd('unpunish','me,'..b) base.pcmd('invis','me') end)
	azu:Command("local","ufo",{},function(a)local b=tostring(a)base.pcmd('dog',b)base.pcmd('creeper',b)base.pcmd('removelimbs',b)azu:Chat("paint "..b.." black")azu:Chat("name "..b.." Ufo")base.pcmd('removehats',b)base.pcmd('spin',b)wait(0.25)base.pcmd('fly',b)end)
	azu:Command("local","taxi",{},function(a)local b=tostring(a)base.pcmd('dog',b)base.pcmd('removelimbs',b)base.pcmd('creeper',b)azu:Chat("name "..b.." Taxi")base.pcmd('gold',b)azu:Chat("speed "..b.." 100")base.pcmd('god',b)base.pcmd('removehats',b)end)
	azu:Command('local','spawn',{"setspawnpoint"},function(a)local b=LP.Character.HumanoidRootPart.CFrame;base.move(base.WSfolder.Spawn1,b)base.move(base.WSfolder.Spawn2,b)base.move(base.WSfolder.Spawn3,b)end)
	...
	azu:Command("local","block",{"blacklist"},function(a)local b=azu:GetPlayer(tostring(a))if b~="None"then table.insert(base.VariableService.players_kick,b)azu:Notification("Blocked "..b)else azu:Notification('No Target')end end)
	azu:Command('local','unblock',{"unblacklist"},function(a)local b=tostring(a)for c,d in pairs(Players:GetChildren())do stringlower=string.lower(d.Name)stringsub=string.sub(stringlower,1,#b)if b==stringsub then for a,e in pairs(base.VariableService.players_kick)do if e==d.Name then table.remove(base.VariableService.players_kick,a)azu:Notification("Unblocked "..d.Name)end end end end end)
	azu:Command('local','slock',{},function(a)local b=azu:GetPlayer(tostring(a))if b~="None"then if table.find(base.VariableService.serverlock_whitelist_players,b)then table.remove(base.VariableService.serverlock_whitelist_players,b)azu:Notification('Removed | '..b)else table.insert(base.VariableService.serverlock_whitelist_players,b)azu:Notification('Added | '..b)end else azu:Notification('No Target')end end)
	azu:Command('local',"voidkill",{"killvoid","setspawnvoid"},function(a) local b = CFrame.new(Vector3.new(1,-50000,1)) base.move(base.WSfolder.Spawn1, b) base.move(base.WSfolder.Spawn2, b) base.move(base.WSfolder.Spawn3, b) end)
	azu:Command('local','detect',{},function(a) base.Status.detect = true end)
	...
	azu:Command('local','clearlogs',{"clrlogs","clearlog","clrlog"},function(a) for c = 1,20 do base.pcmd('ff','despacitolovexll') end end)
	azu:Command('local','blacklistkick',{"bk","kickblacklist"},function(a)local b=azu:GetPlayer(tostring(a))if b~="None"then azu:Chat(azu.ChatEvent.prefix..'lk '..b)table.insert(base.VariableService.players_crash,b)azu:Notification("Blacklisted "..b)else azu:Notification('No Target')end end)
	azu:Command('local','unblacklistkick',{"unbk","unkickblacklist"},function(a)local b=azu:GetPlayer(tostring(a))if b~="None"then for a,c in pairs(base.VariableService.players_crash)do if c==b then table.remove(base.VariableService.players_crash,a)azu:Notification("UnBlacklisted "..b)end end else azu:Notification('No Target')end end)
	azu:Command('local','forcekick',{"fk","kickforce"},function(a) local b=azu:GetPlayer(tostring(a)) if b ~="None" then base.Kick(3,b) else azu:Notification('No Target') end end)
	azu:Command('local','localkick',{"lk","kicklocal"},function(a) local b=azu:GetPlayer(tostring(a)) if b ~="None" then base.Kick(2,b) else azu:Notification('No Target') end end)
	azu:Command('local','clientcrash',{"cc","crashclient","kickclient","clientkick"},function(a) local b=azu:GetPlayer(tostring(a)) if b ~="None" then base.Kick(1,b) else azu:Notification('No Target') end end)
	azu:Command('local','ragdoll',{"rd"},function(a) base.pcmd('punish',a) azu:Chat('speed '..a..' inf') base.pcmd('unpunish',a) end)


	Mouse.Button1Down:Connect(function()pcall(function()local a=Mouse.Target.Parent.Parent;for b,c in pairs(Players:GetPlayers())do pcall(function()if Mouse.Target.Parent:FindFirstChild("Humanoid")then a=Mouse.Target.Parent end end)end;if a~=nil and Players:FindFirstChild(a.Name)then if base.config.clickexplode.Value==true then base.pcmd('explode',a.Name)end;if base.config.clickkill.Value==true then azu:Chat('health '..a.Name..' 0')end;if base.config.clickfling.Value==true then base.pcmd('fling',a.Name)end end end)end)

elseif exploit.GID == azu.comp.evade then
	local as = UiPage:Category("Protection")
	local asc = UiPage:Category("Build")
	local sc = UiPage:Category("Special")
	local evade = {
		runbots = {},
		loadedbuilds = {},
		connections = {},
...
for _,v in pairs(azu.CoreDocumentation) do UiPage:CreateDocumentation(v[1],v[2]) end
for _,v in pairs(azu.ChatEvent.Service) do UiBase.c_2:Label(tostring(azu.ChatEvent.prefix..v.nm)) end
