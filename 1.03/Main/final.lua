...
function azu:ChatSystemEvent(a)game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(b)if game:GetService("Players"):FindFirstChild(a.Name)and b.FromSpeaker==a.Name then for c,d in pairs(azu.ChatEvent.Service2)do local e=string.len(d.nm)local f=string.sub(tostring(b.Message),e+2)if d.target=="local"and b.FromSpeaker==game.Players.LocalPlayer.Name then if string.sub(tostring(b.Message),0,e)==d.nm then coroutine.wrap(d.call)(f)end elseif d.target=="other"and b.FromSpeaker~=game.Players.LocalPlayer.Name then if string.sub(tostring(b.Message),0,e)==d.nm then coroutine.wrap(d.call)(f)end elseif d.target=="everyone"then if string.sub(tostring(b.Message),0,e)==d.nm then coroutine.wrap(d.call)(f)end end end;for g,h in pairs(azu.ChatEvent.Service)do local e=string.len(azu.ChatEvent.prefix)+string.len(h.nm)local f=string.sub(tostring(b.Message),e+2)if h.target=="local"and b.FromSpeaker==game.Players.LocalPlayer.Name then if string.sub(tostring(b.Message),0,e)==azu.ChatEvent.prefix..""..h.nm..""then coroutine.wrap(h.call)(f)end elseif h.target=="other"and b.FromSpeaker~=game.Players.LocalPlayer.Name then if string.sub(tostring(b.Message),0,e)==azu.ChatEvent.prefix..""..h.nm..""then coroutine.wrap(h.call)(f)end elseif h.target=="everyone"then if string.sub(tostring(b.Message),0,e)==azu.ChatEvent.prefix..""..h.nm..""then coroutine.wrap(h.call)(f)end end end end end)end
...
azu:Run(true,function()
	...
	elseif id.game == azu.comp.skywars then
		...
		as:Button("Tokio Custom Theme",function()
			loadstring(game:HttpGet('https://raw.githubusercontent.com/DaFrenchTokio/Azurite/main/lib/Skywars/custom_theme.lua',true))()
		end)
...
Check Chat For Link
		...
		azu:Command('local','block',function(a)local b=tostring(a)azu:Chat("stun "..khols.protectcmd().." "..b)azu:Chat("freeze "..khols.protectcmd().." "..b)azu:Chat("thaw "..khols.protectcmd().." "..b)end)
		azu:Command('local','floppa',function(a)azu:Chat('char me 17513')azu:Chat('shirt me 9582295098')loadstring(game:HttpGet('https://raw.githubusercontent.com/DaFrenchTokio/Azurite/main/lib/Khols%20Admin%20House/floppa.lua',true))()azu:Wait()azu:Chat('removehead me')end)
		azu:Command('local','color',function(a)choice=tostring(a)azu:Chat("gear me 00000000000000000018474459")if choice=="original"then loadstring(game:HttpGet("https://raw.githubusercontent.com/DaFrenchTokio/Azurite/main/lib/Khols%20Admin%20House/color_original.lua",true))()elseif choice=="random"then game.Players.LocalPlayer.Character.Humanoid:EquipTool(game.Players.LocalPlayer.Backpack.PaintBucket)for b,c in pairs(game:GetService("Workspace"):GetDescendants())do coroutine.resume(coroutine.create(function()if c:IsA("Part")then local d={["Part"]=c,["Color"]=Color3.new(math.random(0,255),math.random(0,255),math.random(0,255))}game:GetService("Workspace")[game.Players.LocalPlayer.Name].PaintBucket:WaitForChild("Remotes").ServerControls:InvokeServer("PaintPart",d)end end))end end end)
		...
		azu:Command('local','fixrc',function(a) loadstring(game:HttpGet('https://raw.githubusercontent.com/DaFrenchTokio/Azurite/main/lib/Khols%20Admin%20House/fixrc.lua',true))() end)
		azu:Command('local','kick',function(a)local b=tostring(a)for c,d in pairs(game.Players:GetChildren())do stringlower=string.lower(d.Name)stringsub=string.sub(stringlower,1,#b)if b==stringsub then table.insert(khols.VariableService.players_kick,d.Name)azu:Notification("Kicked "..d.Name)end end end)
		azu:Command('local','unkick',function(a)local b=tostring(a)for c,d in pairs(game.Players:GetChildren())do stringlower=string.lower(d.Name)stringsub=string.sub(stringlower,1,#b)if b==stringsub then for a,e in pairs(khols.VariableService.players_kick)do if e==d.Name then table.remove(khols.VariableService.players_kick,a)azu:Notification("Unkicked "..d.Name)end end end end end)
	...
	elseif id.game == azu.comp.evade then
		...
		function evade.ko()
			loadstring(game:HttpGet(("https://raw.githubusercontent.com/DaFrenchTokio/Azurite/main/lib/Evade/gui1.lua"),true))()
			evade:GetBots()
		...
		function evade.loadbuild()
			local file = string.split(readfile("evade_custom_build.lua"), "\n")
		...
		azu:Loop(true,function()
			if evade.config.autotickets.Value == true then
				for i,out in pairs(game.Workspace.Game.Effects.Tickets:GetChildren()) do
				    azu:WaitRoot(game.Players.LocalPlayer.Character).CFrame = out.Mover1.CFrame
				end
			end
			if evade.config.autorevive.Value == true then
				local hum = azu:WaitHum(game.Players.LocalPlayer.Character)
				if hum then
					if hum.Health == 0 then
						evade.respawn()
					end
