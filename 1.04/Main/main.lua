...
function azu:ChatSystemEvent(a)
	game.Players.LocalPlayer.PlayerGui.Chat.Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar.FocusLost:Connect(
		function(msg)
			print(msg)
		end
	)
    game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(
        function(b)
            if game:GetService("Players"):FindFirstChild(a.Name) and b.FromSpeaker == a.Name then
            	msg = b.Message
            	tablemsg = string.split(msg, " ")
                for _, v in pairs(azu.ChatEvent.Service2) do
                    local e = string.len(v.nm)
                    local f = string.sub(tostring(msg), e + 2)
                    if v.target == "local" and b.FromSpeaker == game.Players.LocalPlayer.Name then
                        if string.sub(tostring(msg), 0, e) == v.nm then
                            coroutine.wrap(v.call)(f)
                        end
                    elseif v.target == "other" and b.FromSpeaker ~= game.Players.LocalPlayer.Name then
                        if string.sub(tostring(msg), 0, e) == v.nm then
                            coroutine.wrap(v.call)(f)
                        end
                    elseif v.target == "everyone" then
                        if string.sub(tostring(msg), 0, e) == v.nm then
                            coroutine.wrap(v.call)(f)
                        end
                    end
                end
                for _, v in pairs(azu.ChatEvent.Service) do
                    local e = string.len(azu.ChatEvent.prefix) + string.len(v.nm)
                    local f = string.sub(tostring(msg), e + 2)
                    fullcmd = azu.ChatEvent.prefix .. "" .. v.nm .. ""
                    if v.target == "local" and b.FromSpeaker == game.Players.LocalPlayer.Name then
                        if string.sub(tostring(msg), 0, e) == fullcmd and tablemsg[1] == fullcmd then
                            coroutine.wrap(v.call)(f)
                        end
                    elseif v.target == "other" and b.FromSpeaker ~= game.Players.LocalPlayer.Name then
                        if string.sub(tostring(msg), 0, e) == fullcmd then
                            coroutine.wrap(v.call)(f)
                        end
                    elseif v.target == "everyone" then
                        if string.sub(tostring(msg), 0, e) == fullcmd then
...
azu:Run(true,function()
	...
	elseif id.game == azu.comp.scp3008 then
	            ...
	            if detected == false then
	                ...
	                if char then
	                    ...
	                    if hum then
	                        detected = true
	                        location = hum
	                        game.Players.LocalPlayer.Character:WaitForChild('HumanoidRootPart').CFrame = location.CFrame
	                    end
	                end
	            end
	        end
		...
		azu:Command('local','tp',function(State)
...
Check Chat For Link
		...
		azu:Command('local','gearify',function(a)
			...
			if handle then
				...
				azu:Chat('size me 1.18')
			end
		end)
		azu:Command('local','test',function(a)
			local b=azu:GetPlayer(tostring(a))
			if b ~="None" then
				azu:GetRoot(game.Players.LocalPlayer.Character).Anchored = true
				azu:Chat("pm/"..b.." werape @ children")
			    azu:Chat("pm "..b.." werape @ children")
				azu:Chat("respawn "..khols.protectcmd().." damixoooo_me")
				azu:Chat('ff me_I_Love_Azurite_And_Tokio')
				azu:Chat("respawn "..khols.protectcmd().." "..b)
				...
				for i = 1,7 do azu:Chat("gear me 25741198") end
				looping = true
				spawn(function()
					while true do wait()
						...
						local PlayerHumanoid = game.Players[b].Character:WaitForChild('HumanoidRootPart')
						local TargetHumanoid = game.Players.LocalPlayer.Character:WaitForChild('HumanoidRootPart')
				...
				wait(.2)
				while true do wait()
					local count = #game.Players.LocalPlayer.Backpack:GetChildren()
					if count > 5 then 
						break
					end
				end
				while true do wait(.2)
					...
					if count > 6 and game.Players[b].Character:FindFirstChild('HotPotato') then
						break
					end
				end
			    wait(.5)
		        ...
		        azu:Chat("removetools "..khols.protectcmd().." damixoooo_me")
		        azu:Chat("respawn "..khols.protectcmd().." damixoooo_me")
		        wait(3)
		        azu:Chat('clr I_Love_Azurite_And_Tokio')
		        azu:Chat("respawn "..khols.protectcmd().." "..b)
		        azu:Chat("size "..b.." 0.5")
		        azu:Chat("freeze "..khols.protectcmd().." "..b)
		        azu:Chat("rainbowify "..khols.protectcmd().." "..b)
		        azu:Chat("name "..b.." 💀💀💀💀💀💀\n💀💀💀💀💀💀\n💀💀💀💀💀💀")
			else
				azu:Notification('No Target')
			end
